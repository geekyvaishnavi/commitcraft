# PRD: commitcraft — AI Conventional Commit Rewriter
...
2. `prepare-commit-msg` hook fires → commitcraft reads `git diff --staged` + existing draft message (if any)
3. commitcraft calls LLM with diff + draft + few-shot Conventional Commit examples
...
- `prepare-commit-msg` git hook, installed via `npx commitcraft init` (writes hook into `.git/hooks/`)
- Single LLM provider (Claude Haiku via Anthropic API — fast, cheap, good enough for this)
- API key via env var (`COMMITCRAFT_API_KEY`) — no config file yet
...
- `.commitcraft.yml` per-repo custom types/scopes
...
- `npm install -g commitcraft && commitcraft init` works in a fresh repo
...
- AM: hook install logic — `commitcraft init` writes a `prepare-commit-msg` script into `.git/hooks/` that shells out to your CLI
